...
if ( current_theme_supports( 'comments' ) ) {
	...
	*/
	function x5_comment_fields($fields) {
	...
	add_filter( 'comment_form_default_fields', 'x5_comment_fields' );

	if ( ! function_exists( 'x5_comment' ) ) :

		/**
		 * Template for comments, without pingbacks or trackbacks
		 * Based on Twenty Eleven Theme
		 */
		function x5_comment($comment, $args, $depth) {
            ...
            <div class="user-comment" id="comment-<?php comment_ID(); ?>">
              ...
              <ul class="meta">
				...
				<?php comment_text(); ?> <?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply', X5 ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
